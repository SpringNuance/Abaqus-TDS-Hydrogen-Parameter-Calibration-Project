...
def main_train_GP_model(global_configs, stage4_outputs):
    ...
    GP_model_object = GaussianProcess(model_config, all_paths, train_X, train_Y)
    GP_model_object.train_model()
    ...
    if current_iteration_index > 0:
        ...
        GP_model_object.save_model(f"{models_path}/iteration_{current_iteration_index}/{model_name}")
    else:
        ...
        GP_model_object.save_model(f"{models_path}/initial/{model_name}")
    
    stage5_outputs = {
        "GP_model_object": GP_model_object
    }
