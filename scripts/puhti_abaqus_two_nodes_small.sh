...
fullpath=$(sed -n 1p scripts/array_file.txt) 
cd ${fullpath}
...
abq2023 job=geometry cpus=$SLURM_NTASKS scratch=tmp_$SLURM_JOB_ID -verbose standard_parallel=all mp_mode=mpi interactive
